...
(function () {
  ...
  const CONFIG = {
    ...
    // NEW: Brightness/Opacity multiplier (0.2 to 1.0, default 1.0)
    brightness: 1.0,
  };

  // Inside the IIFE, right after CONFIG is defined
  window.CONFIG = CONFIG;

  // Load brightness from localStorage
  const savedBrightness = localStorage.getItem('star-brightness');
  if (savedBrightness !== null) {
    CONFIG.brightness = Math.max(0.2, Math.min(1.0, parseInt(savedBrightness) / 200));
  } else {
    ...
    class Star {
      ...
      draw(ctx, color) {
        // Apply brightness to the opacity
    ...
    function drawConnections(ctx, color) {
      ...
      for (let i = 0; i < stars.length; i++) {
        for (let j = i + 1; j < stars.length; j++) {
          ...
          if (distance < maxDist) {
            const opacity = Math.min(maxOpacity * (1 - (distance / maxDist)) * brightness, 1.0);
            ctx.beginPath();
