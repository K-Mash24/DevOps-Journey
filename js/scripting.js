...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const SCRIPTING_OVERVIEW = {
    purpose: {
      title: '📌 Purpose',
      description: [
        'This pillar builds a foundation in automation and scripting — the backbone of DevOps. You will learn Python and Bash to automate repetitive tasks, interact with APIs, parse data, and build robust, maintainable scripts.',
        'Every concept here is studied independently of cloud tooling. The knowledge transfers directly to infrastructure as code, CI/CD pipelines, container orchestration, and monitoring stacks — scripting is the glue that holds the entire DevOps toolchain together.'
      ]
    },
    objectives: [
      'Write Python scripts with variables, data types, control flow, and functions',
      'Use Python modules and handle errors gracefully with try/except',
      'Read and write files using context managers (with open())',
      'Write Bash scripts with shebang, variables, conditionals, and loops',
      'Use advanced Bash features: arrays, argument parsing, exit codes, and traps',
      'Understand REST API concepts: HTTP methods, status codes, headers, and endpoints',
      'Make API requests in Python using the requests library',
      'Parse JSON data using Python\'s json module and jq in Bash',
      'Apply automation patterns: scheduling, idempotency, logging, and error handling'
    ],
    keyConcepts: [
      { term: 'Python syntax', definition: 'Indentation-defined blocks. Variables, data types, control flow (if/else, for/while)' },
      { term: 'Functions & modules', definition: 'def, return, import — creating and using reusable modules' },
      { term: 'Error handling', definition: 'try/except/finally, raising exceptions, logging errors' },
      { term: 'File I/O', definition: 'open(), read(), write(), context managers (with open() as f)' },
      { term: 'Bash shebang', definition: '#!/bin/bash — tells the system which interpreter to use' },
      { term: 'Bash arrays & args', definition: 'declare -a, $@, $#, shift, getopts for argument parsing' },
      { term: 'REST API (Representational State Transfer)', definition: 'HTTP methods, status codes, headers, and endpoints' },
      { term: 'Python requests', definition: 'import requests; requests.get(), .post(), auth, pagination' },
      { term: 'JSON parsing', definition: 'json.loads(), json.dumps(), jq in bash for CLI parsing' },
      { term: 'Idempotency', definition: 'An operation that can be applied multiple times without changing the result beyond the first application' }
    ],
    stats: [
      { label: 'Sections', value: '1/9 (11%)' },
      { label: 'Topics covered', value: '9 so far (45+ planned)' },
      { label: 'Estimated time', value: '~12-14 hours total' },
    ...
    container.innerHTML = `
      <div class="overview-content" style="margin-bottom: 2rem;">
        ...
        <div class="info-box note">
          ...
          <p style="margin-top: 0.25rem;">All markdown notes are committed to <a href="${SCRIPTING_OVERVIEW.readmeLink}" target="_blank" style="color: var(--accent-secondary);">Great_Cheatsheets/saa-foundation/01-networking/</a>. The <a href="${SCRIPTING_OVERVIEW.readmeDetailLink}" target="_blank" style="color: var(--accent-secondary);">README.md</a> in that folder contains the complete pillar summary, learning objectives, and revision checklist.</p>
        </div>
      </div>
    `;
  }
  ...
  function initFloatingProgressRing() {
    ...
    checkboxes.forEach(cb => {
      cb.addEventListener('change', () => {
        localStorage.setItem(`scripting-section-${cb.dataset.section}`, cb.checked);
        updateFloatingRing();
      });
    });
