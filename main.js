...
// ============================================================
// NETWORKING.HTML SPECIFIC LOGIC
// ============================================================

// Active nav highlight on scroll
const navItems = document.querySelectorAll('.sidebar-nav .nav-item[href^="#"]');
const sections = ['overview', 'checklist', 's1', 's2', 's3', 's4', 's5', 's6', 's7', 'flashcards', 'quiz'];
...
// Accordion
// Assigned to window so inline onclick handlers in HTML can access it
  ...
  searchInput.addEventListener('input', function() {
    ...
    accordions.forEach(acc => {
      ...
      if (text.includes(term)) {
        ...
        highlightText(acc, this.value.trim());
        visibleCount++;
      } else {
...
function renderFlashcards() {
  ...
  grid.innerHTML = FLASHCARDS.map((card) => `
    <div class="flashcard" onclick="this.classList.toggle('flipped')" role="button" aria-label="Flashcard: ${card.term}">
...
// Attached to window so inline onclick handlers in HTML can access them
